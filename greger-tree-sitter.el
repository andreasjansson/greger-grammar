      ...
      (let ((child-type (treesit-node-type child)))
        (cond
         ...
         ((string= child-type "title")
          (setq title (greger-tree-sitter--extract-key child)))
         ((string= child-type "cited_text")
          (setq cited-text (greger-tree-sitter--extract-key child)))
         ((string= child-type "encrypted_index")
          (setq encrypted-index (greger-tree-sitter--extract-key child))))))
