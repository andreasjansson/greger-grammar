      ...
      (let ((child-type (treesit-node-type child)))
        (cond
         ...
         ((string= child-type "title")
          (let ((text (string-trim (treesit-node-text child t))))
            (setq title (if (string-prefix-p "Title: " text)
                           (substring text 7)
                         text))))
         ((string= child-type "cited_text")
          (let ((text (string-trim (treesit-node-text child t))))
            (setq cited-text (if (string-prefix-p "Cited text: " text)
                                (substring text 12)
                              text))))
         ((string= child-type "encrypted_index")
          (let ((text (string-trim (treesit-node-text child t))))
            (setq encrypted-index (if (string-prefix-p "Encrypted index: " text)
                                     (substring text 17)
                                   text)))))))
